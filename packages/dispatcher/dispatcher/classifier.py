...
_EMOJI_SYSTEM = (
    "You are an emoji picker for a chat bot. "
    "If the user's message has social, emotional, or interpersonal flavor, "
    "pick ONE emoji that captures the vibe. Be creative — use the full Unicode emoji range. "
    "But most messages are dry task instructions — for those, respond with just a dash: -\n"
    "Respond with ONLY the emoji or -, nothing else."
)

EMOJI_TIMEOUT = 2  # seconds


async def pick_emoji(message: str) -> str | None:
    """Pick a contextual emoji for a message via a fast LLM call.

    Returns a single emoji character, or None on failure.
    Runs fire-and-forget — failures are silent.
    """
    backend = _get_backend()
    if not backend:
        return None
    ...
    try:
        result = await asyncio.wait_for(
            asyncio.to_thread(
                _call_emoji, backend_name, api_key, message,
            ),
            timeout=EMOJI_TIMEOUT,
        )
        result = result.strip()
        # "-" means no emoji needed; also reject anything too long
        if not result or result == "-" or len(result) > 4:
            return None
        return result
    except Exception:
        return None
...
def _call_emoji(backend: str, api_key: str, message: str) -> str:
    """Make the emoji API call. Runs in a thread."""
    if backend == "anthropic":
        return _call_anthropic_emoji(api_key, message)
    return _call_openai_emoji(api_key, message)


def _call_anthropic_emoji(api_key: str, message: str) -> str:
    payload = json.dumps({
    ...
    resp = urlopen(req, timeout=EMOJI_TIMEOUT)
...
def _call_openai_emoji(api_key: str, message: str) -> str:
    payload = json.dumps({
        "model": "gpt-4o-mini",
        "max_tokens": 5,
        "temperature": 0.8,
        "messages": [
            {"role": "system", "content": _EMOJI_SYSTEM},
